...
> **v1.6.4 (2026-08-16, branch `feat/wave2-engine-fixes`)** — Remediation Wave 2 (Medium backlog) complete: atomic `save_config`, import-safe config load, whisper lock, provider/port/category correctness, engine-switch `on_change` callback, single `_reset_analysis_state()` helper, Clear All no longer deletes sessions, `exif.close()` via `finally`, temp-dir cleanup, lazy preview/duplicate toggles, CLI arg groups + config-synced case-style default. Suite at 333 passing, ruff unchanged.
